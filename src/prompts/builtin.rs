...
pub fn builtin_prompts() -> Vec<SmartPrompt> {
    vec![
        ...
        p(
            ...
            "Audit code for concurrency hazards — shared state, ordering, locks",
            "Audit the following code for concurrency hazards. For each finding give file:line, severity (data race / deadlock / livelock / starvation / TOCTOU / lost update / stale read), and the smallest correct fix.\n\nCheck specifically:\n1. Shared mutable state crossing thread / task boundaries.\n2. Locks held across .await points (in async Rust this is a compile error; in other langs flag held-while-blocking).\n3. Read-modify-write sequences that need atomic / CAS / mutex.\n4. Two-phase init where one thread observes a partially-initialized value.\n5. Channel patterns: dropped receivers leaving senders blocked, fan-in starvation, ordering between channels.\n6. Cancellation safety: futures dropped mid-await without state cleanup.\n\nDo NOT speculate about \"theoretical\" races on values that are clearly single-threaded. Only flag hazards reachable from real call paths.\n\n```\n{{input}}\n```",
            "Code Review",
            &["concurrency", "race condition", "review", "thread safety"],
        ),
        p(
            "Review for Resource Leaks",
            "Audit code for file handle, socket, memory, and goroutine leaks",
            "Audit the following code for resource leaks.\n\nFor each leak:\n- File:line where the resource is acquired\n- Path that fails to release it (early return, panic, error in the middle, dropped future)\n- Severity (per-request leak that compounds vs one-shot)\n- The smallest correct fix (RAII guard, defer, try/finally, scope guard, channel close on drop, ...)\n\nCover:\n1. File / socket / DB connection handles — acquired but missing close on every path including error\n2. Long-running tasks / goroutines / threads spawned without a way to terminate\n3. Subscriber / listener registrations without unsubscribe\n4. Strong reference cycles (Arc<RefCell> in Rust, retain cycles in Swift, closures capturing self)\n5. Cancellation: when a parent task is dropped/cancelled, do its children also stop?\n6. Buffers / caches that grow unboundedly\n\n```\n{{input}}\n```",
            "Code Review",
            &["leak", "resources", "memory", "review", "cleanup"],
        ),
        p(
            "Review for API Backwards Compatibility",
        ...
        p(
            ...
            "Find untested branches, error paths, and edge cases in code under test",
            "For the following code (and accompanying tests if shown), enumerate concrete coverage gaps. Don't report \"add more tests\" — report a specific scenario that is currently untested AND a one-line description of how to test it.\n\nFocus on:\n1. Error branches — every Err / catch / null-check that has no failure-path test\n2. Edge cases — empty input, single element, boundary values, max sizes, unicode, time zones, locales\n3. Concurrency — only tested single-threaded? Is there a parallel/contention test?\n4. State machines — are all transitions exercised, or only the happy path?\n5. Integration boundaries — mocked everywhere; is there a test that hits the real boundary?\n6. Regression candidates — bugs the comments mention (\"used to\", \"can no longer\", \"FIXME\") without a test pinning the fix\n\nOutput a table: gap | priority (high/med/low) | test sketch.\n\n```\n{{input}}\n```",
            "Code Review",
            &["testing", "coverage", "review", "gaps"],
        ),
        p(
            "Review for Hot-Path Performance",
            "Identify performance issues without speculative micro-optimizations",
            "Review the following code with performance in mind. Only flag issues you can argue concretely — not micro-optimizations or style preferences.\n\nLook for:\n1. Allocations in tight loops (String concatenation, Vec::new inside iter, format! per element)\n2. O(N²) or worse algorithms where O(N log N) or O(N) is straightforward\n3. Unnecessary cloning (clone before move, clone in hot loop, .to_owned() then immediately discarded)\n4. Synchronous I/O on a hot path (file reads in render, blocking calls in async)\n5. Repeated work across calls that could be cached / precomputed / memoized\n6. Database / network queries that could be batched, pipelined, or eliminated\n\nFor each issue:\n- File:line\n- Why it matters (qualitative — \"runs once per request\" vs \"runs per row\")\n- The specific change (no \"consider optimizing\" — give the exact replacement)\n- Expected order-of-magnitude effect\n\nIgnore everything that runs once at startup or in a cold path.\n\n```\n{{input}}\n```",
            "Code Review",
            &["performance", "optimization", "review", "hot path"],
        ),
        // ── Debugging (5) ──────────────────────────────────────────────────
        p(
            "Diagnose a Memory Leak",
            "Systematic investigation framework for memory growth in production",
            "Help me diagnose a memory leak. I'll describe the symptoms; you walk me through a structured investigation.\n\nProduce a numbered playbook that:\n\n1. CHARACTERIZE the leak from what I told you\n   - Linear growth? Step function? Plateau then crash?\n   - Process-level RSS or heap-only?\n   - Per-request or constant background?\n\n2. NARROW THE WINDOW\n   - Which version introduced it? (`git bisect` plan)\n   - What workload triggers it? (specific endpoint? feature flag? upstream traffic shape?)\n\n3. INSTRUMENT before guessing\n   - Right tool for the runtime: heaptrack / massif / pprof / Instruments / dotnet-counters / V8 heap snapshot / Rust dhat / Go pprof\n   - What to capture: snapshot at low and high RSS, diff them\n   - Allocation profile vs in-use profile — both, not just one\n\n4. INTERPRET the profile\n   - Top retainers: who is holding what alive\n   - Allocation site vs retention site (often different)\n   - Common patterns: unbounded cache, listener never unregistered, closure capturing big object, async task not joined, slab/arena that never frees\n\n5. PROVE THE FIX\n   - Reproducer that demonstrates pre/post growth\n   - Soak test for at least 10x normal request volume\n\nMy situation:\n\n{{input}}",
            "Debugging",
            &["debugging", "memory", "leak", "profiling", "production"],
        ),
        p(
            "Diagnose a Race Condition",
            "Find and prove the race in a concurrent failure",
            "Help me find a race condition. Symptoms below.\n\nWalk me through:\n\n1. CONFIRM IT'S A RACE — not a bug that just looks like one\n   - Frequency vs load (races scale with concurrency)\n   - Reproducibility under sanitizers (TSAN, Helgrind, Loom for Rust, race detector for Go)\n   - Disappears under -j1 / single thread? Strong signal.\n\n2. IDENTIFY THE SHARED STATE\n   - What variable / file / row is being raced on?\n   - Who writes? Who reads? Are writes ordered?\n\n3. IDENTIFY THE INTERLEAVING\n   - Sketch the schedule that produces the bug (T1 reads X=0, T2 writes X=1, T1 writes X=0+1=1 — lost update)\n   - Is it: lost update, stale read, double-execute, TOCTOU, ABA, reorder?\n\n4. PROVE IT WITH A REPRODUCER\n   - Tight loop, multiple threads, assertion that fires when the bug occurs\n   - Run under TSAN / race detector / Loom — let the tool find it\n\n5. PICK THE CORRECT FIX\n   - Lock — when contention is rare and operations are short\n   - Atomic + CAS — for single-word updates\n   - Single-writer model (channel / actor) — for high contention\n   - Compare the cost of each in this specific code path; do not blanket-Mutex everything\n\n6. ADD A TEST that fails without the fix and passes with it\n\nMy situation:\n\n{{input}}",
            "Debugging",
            &["debugging", "race condition", "concurrency", "tsan"],
        ),
        p(
        ...
        p(
            ...
            "Profile, EXPLAIN, and fix a query that's taking too long",
            "Help me diagnose a slow query. I'll show the query and what I know.\n\nWork through this in order:\n\n1. MEASURE THE BASELINE — wall clock, rows scanned, rows returned, plan cost. Don't guess.\n\n2. READ THE PLAN (EXPLAIN ANALYZE / EXPLAIN PLAN / SHOW PLAN)\n   - Sequential scans on tables >10K rows: usually wrong\n   - Hash join with massive build side: missing index forcing nested loop\n   - Sort spilling to disk: work_mem too small or sort can be eliminated\n   - Filter applied AFTER join when it could be pushed down\n\n3. INDEX STRATEGY\n   - Which columns are in WHERE, JOIN, ORDER BY, GROUP BY?\n   - Composite index column order: equality before range\n   - Covering index to avoid heap fetch\n   - Partial index for skewed distributions\n\n4. QUERY REWRITE\n   - EXISTS vs IN — usually EXISTS\n   - Correlated subquery → JOIN or LATERAL\n   - SELECT * → only columns needed (allows index-only scan)\n   - Pagination: keyset over OFFSET for deep pages\n\n5. SCHEMA CHANGES (if needed)\n   - Denormalization for read-heavy hot paths\n   - Materialized view if the query is run often and underlying data changes slowly\n   - Partition for queries always scoped to recent time / a single tenant\n\n6. VERIFY — same EXPLAIN ANALYZE on the new version, compare numbers, not vibes.\n\nMy query:\n\n{{input}}",
            "Debugging",
            &["sql", "slow query", "database", "explain", "indexing"],
        ),
        p(
            "Diagnose a Flaky Test",
            "Find the root cause of intermittently-failing tests",
            "Help me diagnose a flaky test — one that passes sometimes and fails sometimes without code changes.\n\nMost flakes fall into a small number of root causes. Work through them:\n\n1. ORDER DEPENDENCE\n   - Does the test pass alone but fail in suite? (try `cargo test --test flaky -- --test-threads=1` or your equivalent)\n   - Is there shared filesystem / database / process state?\n   - Cure: isolate per-test (tmpdir, fresh DB schema, mock the global)\n\n2. TIMING / RACE\n   - Does it pass on a fast machine and fail on CI?\n   - Sleeps anywhere in the test? Replace with explicit await-condition with timeout.\n   - Wait-for-process-to-start patterns that assume a fixed time?\n\n3. NON-DETERMINISTIC INPUT\n   - HashMap iteration order depended on?\n   - Time.now() / Instant.now() compared against fixed values?\n   - Random not seeded with a fixed seed for reproducibility?\n\n4. EXTERNAL DEPENDENCY\n   - Network call to a real service?\n   - DNS / TLS handshake?\n   - Cure: mock at the boundary, or run a hermetic local instance.\n\n5. RESOURCE EXHAUSTION\n   - Port already in use? File descriptor limit? Concurrent test eats all DB connections?\n   - Cure: dynamic ports, larger pools, semaphore the parallelism.\n\n6. PROVE THE FIX\n   - Run 100x in a loop. If it never fails, the fix held. If it fails twice, you guessed.\n\nFlaky test description / code:\n\n{{input}}",
            "Debugging",
            &["flaky", "tests", "ci", "intermittent", "debugging"],
        ),
        // ── Performance (5) ────────────────────────────────────────────────
        p(
            "Profile a Hot Path",
            "Plan a profiling run that produces actionable findings, not noise",
            "I want to profile a hot code path. Help me plan it so the output is actionable.\n\nFor my situation below, produce:\n\n1. PROFILING TOOL CHOICE — pick the right one for the runtime; don't suggest 5\n   - CPU sampling: perf / Instruments / dotnet-trace / py-spy / Go pprof / cargo flamegraph\n   - Allocation: heaptrack / dhat / Go pprof heap / .NET dotMemory\n   - Wall-clock vs CPU: which matches my question?\n\n2. WORKLOAD\n   - What input shape will I feed it? (a representative request, not a synthetic loop)\n   - Run for at least 30s of CPU to get statistical signal\n   - Stable load — disable autoscaling / GC tuning during the run\n\n3. WHAT TO LOOK AT FIRST\n   - Top 10 self-time functions: where the program is actually spending time\n   - Do NOT immediately optimize the top function — first ask if it's expected to be hot\n   - Wide flame graph plateaus = single function dominating = clear target\n   - Tall narrow stacks = many layers, fix at the top level not inside\n\n4. COMMON DECEPTIONS\n   - GC time eating CPU: the leak is allocations, not the function with high time\n   - Sleep / wait time inflating wall clock: switch to CPU time\n   - JIT warmup: discard the first N seconds\n\n5. EXIT CRITERIA\n   - Specific metric I'll watch (P50/P99 latency? throughput? RSS?)\n   - Specific delta I want (-30%? -50%? specific budget?)\n\nMy hot path:\n\n{{input}}",
            "Performance",
            &["profiling", "performance", "flamegraph", "optimization"],
        ),
        p(
            "Reduce Allocations",
            "Eliminate allocation pressure in tight loops",
            "Find allocation hot-spots in the following code and propose specific fixes. Allocations are real (a malloc / GC pressure), not theoretical.\n\nFor each:\n\n1. WHERE — file:line of the allocating expression\n2. PER-CALL or PER-LOOP-ITERATION? (a one-time alloc at startup is not interesting; one per row is)\n3. THE FIX — be specific:\n   - String concatenation in a loop → preallocate with capacity, write into it\n   - Vec::new in a loop → reuse a single Vec, .clear() between iterations\n   - format! / sprintf for a known shape → write_fmt to a reused buffer\n   - clone() before move → restructure to move directly\n   - Box / Arc where ownership could be a reference\n   - Iterators collecting to Vec when a fold or for_each suffices\n   - String → &str where lifetimes allow\n4. WATCH OUT — don't replace clarity with allocation tricks unless this code is genuinely hot. Profile first.\n\n```\n{{input}}\n```",
            "Performance",
            &["allocations", "performance", "memory", "optimization"],
        ),
        p(
            "Optimize Frontend Performance",
            "Improve Core Web Vitals (LCP, CLS, INP) and runtime perf",
            "Audit the following web page / component / bundle and propose specific improvements ordered by impact on Core Web Vitals.\n\nGo through, in this order:\n\n1. LARGEST CONTENTFUL PAINT (LCP) — typically the easiest win\n   - Hero image: served at correct size, AVIF/WebP, fetchpriority=\"high\", preloaded\n   - Render-blocking CSS / JS in <head>: inline critical, defer rest\n   - Font: font-display: swap; preload the .woff2; subset to needed chars\n   - Server response time: TTFB; CDN; edge cache\n\n2. CUMULATIVE LAYOUT SHIFT (CLS)\n   - Images / iframes / ads with no dimensions reserved\n   - Web fonts causing FOIT/FOUT — size-adjust descriptor\n   - Animations on layout-affecting properties (use transform/opacity)\n\n3. INTERACTION TO NEXT PAINT (INP)\n   - Long tasks blocking main thread (>50ms)\n   - Hydration cost on initial load\n   - Heavy event handlers (debounce, virtual lists, web workers)\n\n4. BUNDLE SIZE\n   - Tree shaking working? (sideEffects in package.json, ESM-first deps)\n   - Code splitting at route boundaries\n   - Replacing moment.js / lodash / date-fns full imports with surgical ones\n\n5. RUNTIME\n   - Re-renders: React DevTools Profiler / Lighthouse Performance tab\n   - Memoization: only when the prop is referentially stable\n\nFor each finding: severity (LCP-blocker / minor), the change, expected effect.\n\n```\n{{input}}\n```",
            "Performance",
            &["frontend", "web vitals", "lcp", "cls", "inp", "performance"],
        ),
        p(
            "Optimize Cold Start",
            "Reduce serverless / app startup latency",
            "Help me reduce cold-start latency for the following service. \n\nWalk through the stages of startup and what to cut at each:\n\n1. RUNTIME INITIALIZATION (everything before user code runs)\n   - Language / runtime: lighter alternative? (provided.al2023 vs nodejs20 in Lambda)\n   - Memory configuration: in some platforms more memory = more vCPU = faster init\n   - Architecture: arm64 often colder-start-faster than x86_64\n\n2. APPLICATION INITIALIZATION (top-level code, imports, DI container)\n   - Top-level imports doing real work? Defer.\n   - DI container scanning the world at startup? Eager-load only what's needed for the first request.\n   - Class loaders / reflection: precompile / AOT where possible (GraalVM, .NET ReadyToRun, etc.)\n\n3. NETWORK SETUP\n   - First DB connection: use a connection per cold start, not a pool of 10\n   - First HTTPS handshake: keep-alive, connection reuse if the runtime allows\n   - DNS: cache; some runtimes do not cache by default\n\n4. SECRETS / CONFIG\n   - Secret manager fetch on every cold start: cache aggressively, accept staleness\n   - Multiple round trips: batch into one\n\n5. PLATFORM TRICKS\n   - SnapStart / Lambda Provisioned Concurrency / Cloud Run min-instances\n   - Always weigh cost vs latency budget\n\n6. MEASURE\n   - Init duration metric specifically (not total request time)\n   - Cold-vs-warm percentile gap: aim to close it, not eliminate it\n\nMy service:\n\n{{input}}",
            "Performance",
            &["cold start", "serverless", "startup", "latency", "lambda"],
        ),
        p(
            "Reduce Compile / Build Time",
            "Cut build feedback loop from minutes to seconds",
            "Help me speed up the build / compile loop for the following project.\n\nGo through these in order — earlier wins compound:\n\n1. INCREMENTAL CORRECTNESS\n   - Is the incremental build actually incremental? (cargo check, ts --incremental, gradle build cache, ccache)\n   - Are timestamps preserved across CI runs? (use the actions cache for the right paths)\n\n2. PARALLELISM\n   - Job count tuned for the machine (cargo build -j N, make -j, gradle --parallel)\n   - Don't oversubscribe — 2x cores for I/O-bound, 1x for CPU-bound\n\n3. CACHE\n   - Build cache configured? (Gradle remote cache, Bazel, sccache for Rust/C++)\n   - CI cache hit rate? <80% means the cache key is wrong\n   - Docker layer ordering: rarely-changing dependencies before frequently-changing source\n\n4. DEPENDENCY GRAPH\n   - Cycles or false dependencies forcing rebuilds?\n   - Generated code triggering re-runs of everything?\n   - For Rust: cargo tree --duplicates to find version-skew bloat\n\n5. SOURCE-LEVEL COSTS\n   - Heavy macro expansion / template metaprogramming\n   - Big proc-macros (serde) generating lots of code per derive — split where possible\n   - Unity / lumped builds for C++\n\n6. TEST TIME\n   - Unit tests in seconds, integration in minutes? Right.\n   - Don't run integration on every push — separate workflow\n\nFor each suggestion: estimated savings (gut-feel range), risk, effort.\n\n```\n{{input}}\n```",
            "Performance",
            &["build", "compile time", "ci", "developer productivity"],
        ),
        // ── Migration (5) ──────────────────────────────────────────────────
        p(
            "Plan a Major Framework Upgrade",
            "Stage a multi-version framework upgrade with rollback points",
            "Plan a major framework / library upgrade for the following codebase. Major = breaking changes are expected.\n\nProduce a phased plan:\n\nPHASE 0 — INVENTORY\n- List the framework's breaking changes between current and target versions (read the changelogs).\n- Find every direct usage of the deprecated APIs in the codebase.\n- Find indirect usage — plugins, transitive deps that will need their own bumps.\n\nPHASE 1 — PIN AND BACKFILL\n- Pin the current version to ensure no drift during the upgrade.\n- Backfill missing tests around APIs you'll change. Don't refactor without a safety net.\n\nPHASE 2 — STAGE THE UPGRADE\n- If multiple major versions: do them one at a time. Land each as a release. Don't try N → N+3 in one PR.\n- For each major bump:\n  a. Update the dep\n  b. Apply mechanical migrations (codemod / framework migrator if provided)\n  c. Fix non-mechanical breaks (the cases the codemod can't handle)\n  d. Ensure tests pass\n  e. Ship. Get production confidence before the next bump.\n\nPHASE 3 — ROLL OUT\n- Feature flag the new behavior if any user-visible change.\n- Canary deployment, watch error rates, only then roll to 100%.\n\nPHASE 4 — CLEAN UP\n- Remove compatibility shims.\n- Update docs / READMEs / runbooks.\n\nFor each phase, list the specific files / modules that will change and the rough effort.\n\nMy upgrade:\n\n{{input}}",
            "Migration",
            &["upgrade", "framework", "migration", "breaking changes"],
        ),
        p(
            "Extract a Service from a Monolith",
            "Identify, extract, and cut over a bounded context safely",
            "Help me extract a piece of functionality from a monolith into a separate service. Goal is a safe, reversible cutover — not a big-bang rewrite.\n\nWalk me through:\n\n1. CHOOSE THE BOUNDARY\n   - Pick a piece with stable, narrow contracts and clear ownership.\n   - Bounded contexts that already have well-defined types beat \"this folder is big\".\n   - High-traffic vs low-traffic: low-traffic first, build the operational muscle.\n\n2. CHARACTERIZE THE INTERFACE\n   - Every call into this code: list them. (grep / IDE call graph)\n   - Every call out: list them. These become RPCs after extraction.\n   - Shared database tables: list them. These are the hardest part.\n\n3. STRANGLER FIG SETUP\n   - In the monolith, route the affected calls through an interface (façade) that today calls the in-process implementation.\n   - Behind that interface: ship a stub HTTP/gRPC/queue client that's not yet enabled.\n\n4. DUAL WRITE / SHADOW READ\n   - Stand up the new service. Have it serve a copy of the data.\n   - Send shadow reads from monolith to new service; compare; alert on divergence.\n   - Run for a meaningful soak — at least one week of production traffic shape.\n\n5. CUT OVER\n   - Flip the façade to the new service for read traffic, behind a flag.\n   - Watch error rate, latency, business metrics.\n   - Then writes. Writes are harder if there's shared schema.\n\n6. DECOMMISSION\n   - Remove the in-monolith implementation only after a quiet period.\n   - Delete shared tables / dual-write logic last.\n\nMy monolith piece:\n\n{{input}}",
            "Migration",
            &["microservice", "monolith", "extraction", "strangler fig"],
        ),
        p(
            "Migrate a Database Schema with Zero Downtime",
            "Stage schema changes that don't lock tables or break old code",
            "Plan a zero-downtime schema migration for the following change. Old code and new code will coexist for some window — the migration must support both.\n\nFollow the expand-contract pattern:\n\nPHASE 1 — EXPAND (additive only)\n- Add the new column / table / index — nullable, with default if needed\n- For very large tables: use online DDL (pt-online-schema-change, gh-ost, CREATE INDEX CONCURRENTLY)\n- Backfill in batches, throttle to avoid replication lag, log progress\n- Old code keeps working; new code can read the new shape\n\nPHASE 2 — DUAL-WRITE\n- Update writers to write BOTH old and new shape\n- Verify: count mismatches, divergent values, nulls in new column\n- Run for long enough to cover all writers and any retries\n\nPHASE 3 — DUAL-READ THEN SWITCH\n- Update readers to use the new shape, with a fallback to the old\n- After confidence, drop the fallback\n- Monitor: error rates, business metrics tied to this data\n\nPHASE 4 — CONTRACT (destructive)\n- Stop writing the old shape\n- Drop the old column / table / index\n- THIS IS THE ONLY DESTRUCTIVE STEP. Make it small, separate PR, easy to roll back if you forgot a writer.\n\nFor each phase:\n- The exact SQL\n- The application code change\n- How to verify before moving on\n- The rollback plan\n\nMy schema change:\n\n{{input}}",
            "Migration",
            &["database", "migration", "zero downtime", "expand contract"],
        ),
        p(
            "Migrate JavaScript to TypeScript",
            "Incrementally add types without freezing development",
            "Plan a JS-to-TS migration for the following codebase. Goal is gradual, behind-flag, tests-still-passing — never a multi-week branch.\n\nStaged approach:\n\nPHASE 1 — TOOLCHAIN\n- Add TypeScript with `allowJs: true`, `checkJs: false`, `strict: false`. .js files keep working untouched.\n- Wire `tsc --noEmit` into CI alongside the existing build, NOT as a gate yet.\n- Add per-file overrides for ESLint to handle .ts.\n\nPHASE 2 — LEAVES FIRST\n- Identify modules with no JS callers in the rest of the codebase (or only easy ones).\n- Rename .js → .ts, fix the immediate type errors.\n- Don't enable `strict` yet. Get the .ts file building, then tighten per-file.\n\nPHASE 3 — DEPENDENCY ORDER\n- Convert leaf-by-leaf, working up the dependency graph.\n- For modules with many callers: convert the file to .ts but keep its public types loose (`any` parameters acceptable temporarily) — fix one caller at a time later.\n\nPHASE 4 — TIGHTEN\n- Per-file: enable `strict` one option at a time (`noImplicitAny`, `strictNullChecks`, ...)\n- The order matters: noImplicitAny first reveals the most mistakes.\n\nPHASE 5 — LIBRARY TYPES\n- Replace `@types/foo` with the package's bundled types where they ship them.\n- For deps without types: write minimal `.d.ts` declarations rather than `// @ts-ignore`.\n\nPHASE 6 — GUARDRAIL\n- Once 80%+ of source is TS, flip CI to fail on new .js files (lint rule, not tsconfig).\n\nFor my codebase specifically: which leaves to convert first.\n\n{{input}}",
            "Migration",
            &["typescript", "migration", "javascript", "incremental"],
        ),
        p(
            "Replace an Auth System",
            "Cut over from one identity provider to another without locking users out",
            "Plan an auth system migration (e.g. legacy session cookies → OIDC, one IdP → another, rolling your own → managed). Users must not lose access during the cutover.\n\nFollow this template:\n\n1. FREEZE THE OLD SYSTEM\n   - Don't ship new features against it. Get to a stable baseline.\n\n2. STAND UP THE NEW SYSTEM SIDE-BY-SIDE\n   - User store: do you migrate users, federate (old as IdP for new), or have both?\n   - For password-based migrations: you can NOT rehash without the cleartext. Plan for: (a) migrate-on-next-login, (b) force password reset, or (c) federate-then-deprecate.\n\n3. DUAL-AUTH WINDOW\n   - Both old and new sessions are valid.\n   - New logins create new-system sessions only.\n   - Old sessions naturally age out as they expire.\n\n4. FORCE THE TAIL\n   - After most users have been migrated, set a deadline. Inform users in-product.\n   - On that date: invalidate remaining old sessions, require login (which now uses the new flow).\n\n5. DECOMMISSION\n   - After a quiet period (zero auth attempts on the old system for a week+), remove the old code and its database tables.\n\n6. SECURITY-CRITICAL CONSIDERATIONS\n   - Token shape change → all clients must accept the new shape before you start issuing it\n   - Cookie domain / SameSite / Secure flags — get them right BEFORE the cutover, or you'll lock people out cross-domain\n   - Refresh token rotation logic: well-tested before you flip a single user\n   - Logout: must clear sessions in BOTH systems for the dual-auth window\n   - Rate-limit the old endpoints during cutover so a brute-force on legacy can't compromise migrated users\n\nMy migration:\n\n{{input}}",
            "Migration",
