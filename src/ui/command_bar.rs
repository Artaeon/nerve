...
///
/// Derived from the single command catalog (`crate::commands::catalog`) so the
/// palette can never drift from the inline autocomplete in `completion`.
fn command_prompts() -> Vec<prompts::SmartPrompt> {
    crate::commands::catalog::all()
        .iter()
        .map(|c| prompts::SmartPrompt {
            name: format!("/{}", c.name),
            description: c.description.into(),
            template: format!("/{}", c.name),
            category: format!("Commands: {}", c.category),
